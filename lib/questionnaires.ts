...
import { calculateQuestionnaireProgress, caseSteps, questionnaireLifecycleStatus } from "./questionnaire-definitions.ts";

export type QuestionnaireAnswers = Record<string, string | boolean>;
export type QuestionnaireStatus = "draft" | "in_progress" | "completed";
export type QuestionnaireRecord = { id: string; client_id: string; questionnaire_type: QuestionnaireType; status: QuestionnaireStatus; answers_encrypted: string; section_progress: Record<string, number>; progress_percent: number; respondent_name: string | null; respondent_email: string | null; created_at: string; updated_at: string; last_saved_at: string | null; submitted_at: string | null };
...
export async function saveQuestionnaire(row: QuestionnaireRecord, answers: QuestionnaireAnswers, submit = false, respondent?: { name?: string; email?: string }) {
  const { url } = config(); const progress = calculateQuestionnaireProgress(row.questionnaire_type, answers); const lifecycle = questionnaireLifecycleStatus(row.questionnaire_type, answers); if (submit && progress < 100) throw new Error("Le questionnaire doit être rempli à 100 % avant la soumission."); const now = new Date().toISOString();
  const completed = submit || lifecycle === "completed";
  const body = { answers_encrypted: encryptAnswers(answers), progress_percent: progress, status: completed ? "completed" : lifecycle, last_saved_at: now, submitted_at: completed ? (row.submitted_at || now) : null, respondent_name: respondent?.name || row.respondent_name, respondent_email: respondent?.email || row.respondent_email };
  const r = await checked(await fetch(`${url}/rest/v1/client_questionnaires?id=eq.${row.id}`, { method: "PATCH", headers: dbHeaders("return=representation"), body: JSON.stringify(body) }), "Sauvegarde questionnaire"); const saved = ((await r.json()) as QuestionnaireRecord[])[0];
  const step = row.questionnaire_type === "client_principal" ? "client_questionnaire" : "guarantor_questionnaire"; const caseStatus: CaseStatus = completed ? "completed" : progress > 0 ? "in_progress" : "todo"; await updateCaseProgress(row.client_id, step, caseStatus, completed ? "Questionnaire complété automatiquement" : "Progression synchronisée automatiquement", "questionnaire");
  await touchClientFromQuestionnaire(row.client_id, row.questionnaire_type, answers, now); return saved;
}

async function touchClientFromQuestionnaire(clientId: string, type: QuestionnaireType, answers: QuestionnaireAnswers, now: string) {
  const { url } = config(); const body: Record<string, unknown> = { updated_at: now };
  if (type === "client_principal") { const givenNames = String(answers.given_names || "").trim(), familyName = String(answers.family_name || "").trim(); if (givenNames && familyName) body.full_name = `${givenNames} ${familyName}`; if (answers.email) body.email = String(answers.email); if (answers.phone) body.phone = String(answers.phone); if (answers.residence_country) body.country = String(answers.residence_country); }
  await checked(await fetch(`${url}/rest/v1/admin_clients?id=eq.${encodeURIComponent(clientId)}`, { method: "PATCH", headers: dbHeaders(), body: JSON.stringify(body) }), "Synchronisation du dossier client");
}

export async function getDecryptedQuestionnaires(clientId: string) { const rows = await listQuestionnaires(clientId); return rows.map((row) => ({ row, answers: decryptAnswers(row.answers_encrypted) })); }

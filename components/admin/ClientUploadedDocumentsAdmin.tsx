...
import { Check, Download, Eye, FileText, History, RefreshCw, Search, Trash2, UploadCloud } from "lucide-react";
...
export function ClientUploadedDocumentsAdmin({ clientId, documents }: { clientId: string; documents: ClientUploadedDocument[] }) {
  ...
  return <div className="space-y-6">
    ...
    <div>
      ...
      <div className="space-y-3">{filtered.length ? filtered.map((document) => <DocumentRow key={document.id} document={document} clientId={clientId} onReplace={() => setReplace(document)} onRemove={() => remove(document)} />) : <p className="rounded-2xl bg-ivory p-5 text-sm font-bold text-navy/52">Aucun document ne correspond à cette vue.</p>}</div>
    </div>
...
function DocumentRow({ document, clientId, onReplace, onRemove }: { document: ClientUploadedDocument; clientId: string; onReplace: () => void; onRemove: () => void }) {

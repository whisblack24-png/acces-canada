...
export function ClientUploadedDocumentsAdmin({ clientId, documents,analyses=[] }: { clientId: string; documents: ClientUploadedDocument[];analyses?:DocumentAnalysis[] }) {
  ...
  return <div className="space-y-6">
    ...
    <div>
      ...
      <div className="space-y-3">{filtered.length ? filtered.map((document) => <DocumentRow key={document.id} document={document} analysis={analyses.find(item=>item.upload_id===document.id)} clientId={clientId} onReplace={() => setReplace(document)} onRemove={() => remove(document)} />) : <p className="rounded-2xl bg-ivory p-5 text-sm font-bold text-navy/52">Aucun document ne correspond à cette vue.</p>}</div>
    </div>
...
function DocumentRow({ document,analysis, clientId, onReplace, onRemove }: { document: ClientUploadedDocument;analysis?:DocumentAnalysis; clientId: string; onReplace: () => void; onRemove: () => void }) {

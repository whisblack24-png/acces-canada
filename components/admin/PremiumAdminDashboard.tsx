...
export function PremiumAdminDashboard({ data, tasks, reminders }: { data: DashboardData; tasks: ClientTask[]; reminders: ClientReminder[] }) {
  ...
  const metrics = [
    ["CA aujourd’hui", formatUsd(data.metrics.revenueDay), CircleDollarSign], ["CA cette semaine", formatUsd(data.metrics.revenueWeek), TrendingUp],
    ["CA ce mois", formatUsd(data.metrics.revenueMonth), BarChart3], ["CA cette année", formatUsd(data.metrics.revenueYear), CircleDollarSign],
...
function TrendChart({ rows, metric }: { rows: DashboardData["monthly"]; metric: ChartMetric }) { const max=Math.max(...rows.map((row)=>row[metric]),1); return <div className="flex h-64 items-end gap-2 rounded-2xl bg-ivory p-4 md:gap-3">{rows.map((row)=><div key={row.key} className="group flex h-full flex-1 flex-col justify-end"><div className="relative min-h-1 rounded-t-lg bg-gradient-to-t from-navy to-gold transition-all duration-500 group-hover:from-canada" style={{height:`${Math.max(3,row[metric]/max*100)}%`}} title={`${row.label}: ${metric === "revenue" ? formatUsd(row[metric]) : row[metric]}`}><span className="absolute -top-7 left-1/2 hidden -translate-x-1/2 whitespace-nowrap rounded bg-navy px-2 py-1 text-[9px] font-black text-white group-hover:block">{metric === "revenue" ? formatUsd(row[metric]) : row[metric]}</span></div><span className="mt-2 truncate text-center text-[9px] font-black uppercase text-navy/35">{row.label}</span></div>)}</div>; }

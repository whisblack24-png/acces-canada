...
import { BarChart3, Download, FileText, FolderKanban, Landmark, UploadCloud, UsersRound, type LucideIcon } from "lucide-react";
...
function MetricCard({ label, value, icon: Icon }: { label: string; value: string; icon: LucideIcon }) {
  return (
    <div className="rounded-[1.5rem] bg-white p-6 shadow-premium">
      ...
      <p className="mt-5 text-xs font-black uppercase tracking-[0.18em] text-navy/42">{label}</p>
...
function ReportPanel({ title, rows, empty }: { title: string; rows: ReportRow[]; empty: string }) {
  ...
  return (
    <section className="rounded-[2rem] bg-white p-6 shadow-premium md:p-7">
...
export default async function AdminReportsPage() {
  ...
  const metrics = [
    ...
    { label: "Documents générés", value: String(report.totals.generatedDocuments), icon: FileText },
    { label: "Documents envoyés", value: String(report.totals.uploadedDocuments), icon: UploadCloud },
    { label: "Paiements", value: String(report.totals.payments), icon: Landmark },
    ...
    <AdminShell>
      <div className="space-y-8">
        <section className="rounded-[2rem] bg-navy p-7 text-white shadow-premium md:p-10">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <h1 className="mt-4 font-display text-4xl font-black md:text-6xl">Rapports administratifs.</h1>
              <p className="mt-5 max-w-3xl leading-8 text-white/66">
                Analysez les clients, dossiers, documents, paiements disponibles et indicateurs clés d'Accès Canada depuis un
        ...
        <section className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
          <div className="rounded-[2rem] bg-white p-6 shadow-premium md:p-7">
            <h2 className="font-display text-2xl font-black text-navy">Dossiers récents</h2>
            <div className="mt-5 overflow-hidden rounded-3xl border border-navy/8">
              <div className="grid grid-cols-[1.2fr_0.8fr_0.7fr] bg-navy px-4 py-3 text-xs font-black uppercase tracking-[0.16em] text-white/72">
                <span>Client</span>
                <span>Service</span>
                <span>Statut</span>
              </div>
              {report.recentClients.length ? (
                report.recentClients.map((client) => (
                  <div key={client.id} className="grid grid-cols-[1.2fr_0.8fr_0.7fr] gap-3 border-t border-navy/8 px-4 py-4 text-sm font-bold text-navy/72">
                    <span className="text-navy">{client.full_name}</span>
          ...
          <div className="rounded-[2rem] bg-white p-6 shadow-premium md:p-7">
            ...
            <div className="mt-5 space-y-3">
              <div className="rounded-2xl bg-ivory p-4">
                <p className="text-xs font-black uppercase tracking-[0.18em] text-navy/42">Documents reçus</p>
                <p className="mt-2 text-3xl font-black text-navy">{report.totals.documentsReceived}</p>
              </div>
              <div className="rounded-2xl bg-ivory p-4">
                <p className="text-xs font-black uppercase tracking-[0.18em] text-navy/42">Documents manquants</p>
              ...
              <div className="rounded-2xl bg-gold/15 p-4">
                <p className="text-xs font-black uppercase tracking-[0.18em] text-navy/42">Dernière génération</p>
                <p className="mt-2 text-sm font-black text-navy">
                  {report.generatedDocuments[0]
                    ? new Date(report.generatedDocuments[0].created_at).toLocaleDateString("fr-CA")
                    : "Aucun document"}
                </p>
              </div>
            </div>

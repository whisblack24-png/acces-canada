...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (body.email) {
      ...
      await sendSmtpMail({ host, port, secure: process.env.SMTP_SECURE === "true" || port === 465, user, pass, from: process.env.SMTP_FROM || user, to: client.email, subject: "Accès Canada · Votre questionnaire sécurisé", text: content.text, html: content.html, replyTo: process.env.SMTP_REPLY_TO || process.env.SMTP_FROM || user });
    }
